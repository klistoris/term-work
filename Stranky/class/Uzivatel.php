...
class Uzivatel
{
    ...
    public function ucastnimSeUdalosti($id_udalost) : bool{
        ...
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM osoba_jede_poskytnutym_vozem 
                            WHERE vuz_na_udalost_id IN (SELECT id FROM auto_jede_na_udalost WHERE udalost_id = :id_udalost) AND 
                            osoba_id = :id_osoba");
    ...
    public function mojeAuta($id_auto) : bool{
        $stmt_znaky = $this->conn->prepare("SET NAMES 'utf8'");
        $stmt_znaky->execute();
        $id_osoba = $_SESSION['identity']['idOsoba'];
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM auto WHERE majitel = :id_osoba AND idauto = :id_auto");
